      ...
      <div className="space-y-6">
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1 dark:text-gray-400">Your exam results on record.</p>
          </div>
          <button
            type="button"
            onClick={handleAskAI}
          ...
          <div className="bg-white border border-gray-200 rounded-lg divide-y divide-gray-100 dark:bg-gray-900 dark:border-gray-700 dark:divide-gray-800">
            {results.map((r) => (
              <div key={r.id} className="px-5 py-4 flex items-center justify-between">
                <div>
                  <div className="font-medium text-gray-900 dark:text-gray-100">{r.examName}</div>
                  <div className="text-xs text-gray-500 mt-1 dark:text-gray-400">{r.unitName} · {r.termName}</div>
                </div>
                <div className="text-sm text-gray-800 dark:text-gray-200">{r.score}/{r.maxScore}</div>
              </div>
            ))}
          </div>
        )}
