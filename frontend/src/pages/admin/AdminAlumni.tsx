  ...
  return (
    <PortalLayout title="Alumni">
      <div className="space-y-6">
        <div>
          <h2 className="text-2xl font-bold text-gray-900 dark:text-gray-100">Alumni</h2>
        ...
        <form onSubmit={handleGraduate} className="bg-white border border-gray-200 rounded-lg p-5 space-y-3 dark:bg-gray-900 dark:border-gray-700">
          <h3 className="font-semibold text-gray-900 dark:text-gray-100">Graduate a Student</h3>

          <div className="relative">
            <input
              className="border border-gray-300 rounded-lg pl-3 pr-9 py-2 text-sm w-full dark:border-gray-600"
            ...
            {dropdownOpen && (
              <div className="absolute z-10 mt-1 w-full bg-white border border-gray-200 rounded-lg shadow-lg max-h-56 overflow-y-auto divide-y divide-gray-100 dark:bg-gray-900 dark:border-gray-700 dark:divide-gray-800">
                {loading && <p className="text-sm text-gray-400 p-3 dark:text-gray-500">Loading students...</p>}
                {!loading && filteredStudents.length === 0 && (
                  <p className="text-sm text-gray-400 p-3 dark:text-gray-500">No matching students</p>
                )}
                {!loading &&
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              ...
              {!loading && alumni.length === 0 && (
                <tr><td colSpan={6} className="px-4 py-6 text-center text-gray-400 dark:text-gray-500">No alumni yet</td></tr>
              )}
              {alumni.map((a) => (
