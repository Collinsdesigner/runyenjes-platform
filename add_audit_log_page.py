#!/usr/bin/env python3
"""
Adds the Audit Log admin page:
  1. Creates frontend/src/pages/admin/AdminAuditLog.tsx (same pattern as
     AdminUsers.tsx -- filter, table, pagination)
  2. Registers the /admin/audit-log route in App.tsx
  3. Adds an "Audit Log" link to the Management section of the ADMIN
     sidebar in PortalLayout.tsx
...
# ---------------------------------------------------------------------------
def create_page():
    path = ROOT / "frontend" / "src" / "pages" / "admin" / "AdminAuditLog.tsx"
    if path.exists():
...
def patch_app():
    ...
    if "AdminAuditLog" in text:
        print("[OK] App.tsx: AdminAuditLog already wired, skipping")
        return

    import_marker = "import AdminUsers from './pages/admin/AdminUsers';"
    if import_marker not in text:
        fail("Could not find the AdminUsers import in App.tsx to anchor the new import near")
    text = text.replace(
        import_marker,
    ...
    route_marker = '                <Route path="/admin/users" element={<AdminUsers />} />'
    if route_marker not in text:
        fail("Could not find the /admin/users route in App.tsx to anchor the new route near")
    text = text.replace(
        route_marker,
...
# ---------------------------------------------------------------------------
# 3. PortalLayout.tsx (sidebar link)
# ---------------------------------------------------------------------------
def patch_sidebar():
    path = ROOT / "frontend" / "src" / "components" / "portal" / "PortalLayout.tsx"
    text = read(path)

    if "/admin/audit-log" in text:
        print("[OK] PortalLayout.tsx: Audit Log sidebar link already present, skipping")
        return
    ...
    if marker not in text:
        fail("Could not find the ADMIN 'Management' sidebar section to add the Audit Log link to")
    new_block = """    {
      title: 'Management',
      items: [
        { label: 'Reports', path: '/admin/reports', icon: '📊' },
...
if __name__ == "__main__":
    ...
    patch_app()
    patch_sidebar()
    print("[OK] Audit Log admin page wired up.")
